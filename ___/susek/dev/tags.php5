...
*/		
	...
	$tag_maxsize = max($tag_list_freq);
	foreach($tag_list as $tag_id=>$tag){
		$tag_url = urlencode($tag);
		$tag_linksize = 8 +  ceil(7 * $tag_list_freq[$tag_id] / $tag_maxsize);
		echo "<a style='font-size:{$tag_linksize}pt' href='http://susek.ru/screbi/tag/{$tag_url}'>{$tag} ({$tag_list_freq[$tag_id]})</a> \n";
	}
